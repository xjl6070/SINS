...
%
% Author:   Matt Fig with an improvement by Bruno Luong (See comments)
...
flg = false; % In case we need to transpose.
...
if cB > ceil(1.5*rB)
    ...
    [rB,cB] = size(B);
    flg = true; % For the correct output at the end.
end

Atv = A(:).';  % Make a single row vector for strfind
clear A  % We don't need A any more, for large problems, free up memory.
...
vct = strfind(Atv,B(:,1).');  % Look for occurrences of the first col of B.
% Next eliminate wrap-arounds, this line was much improved by Bruno Luong.
idx = vct(mod(vct-1,rA) <= rA-rB);  
cnt = 2;  % Counter for the while loop.

while ~isempty(idx) && cnt<=cB
    vct = strfind(Atv,B(:,cnt).');  % Look for successive columns.
...
if flg  % We must get the index to A from index to A'
